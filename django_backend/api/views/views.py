...
from api.models import City, Tourist, Country, Citizen
from api.serializers import TouristCitySerializer, CountrySerializer2, CitizensByAvgPopulationSerializer, \
    CityCitizenSerializer, CitiesByAvgAgeSerializer, TouristsByAvgMoneySerializer
...
class TouristCitiesList(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        countries = countries.filter(countryTourist_id=pk)

        serializer = TouristCitySerializer(countries, many=True)
...
class TouristsByAvgMoney(APIView):
    def get(self, request):
        ...
        serializer = TouristsByAvgMoneySerializer(queryset, many=True)
        return Response(serializer.data)
...
#     def get(self, request):
#         queryset = City.objects.annotate(avg_cityPopulation=Avg
#         ('cityCountry__countryPopulation')).order_by('-avg_countryPopulation')
...
#     def get(self, request):
#         queryset = Citizen.objects.annotate(avg_countryPopulation=Avg
#         ('citizenCity__cityCountry__countryPopulation')).order_by('-avg_countryPopulation')
#         serializer = CitizensByAvgPopulationSerializer(queryset, many=True)
#         return Response(serializer.data)
